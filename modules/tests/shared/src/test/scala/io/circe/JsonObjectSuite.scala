...
class JsonObjectSuite extends CirceSuite {
  property("JsonObject.fromIterable should drop all but the last instance when fields have the same key") {
    Prop.forAll { (key: String, values: List[Json]) =>
  ...
  test("JsonObject.fromFoldable should match JsonObject.fromIterable") { (fields: List[(String, Json)]) =>
  ...
  test("JsonObject.apply should match JsonObject.fromIterable") { (fields: List[(String, Json)]) =>
    assert(JsonObject(fields: _*) === JsonObject.fromIterable(fields))
  }

  test("apply should find fields if they exist") { (fields: List[(String, Json)], key: String) =>
    val result1 = JsonObject.fromIterable(fields)
    val result2 = JsonObject.fromFoldable(fields)
  ...
  test("contains should find fields if they exist") { (fields: List[(String, Json)], key: String) =>
    val result1 = JsonObject.fromIterable(fields)
    val result2 = JsonObject.fromFoldable(fields)
  ...
  test("size should return the expected result") { (fields: List[(String, Json)]) =>
    val result1 = JsonObject.fromIterable(fields)
    val result2 = JsonObject.fromFoldable(fields)
  ...
  test("isEmpty should return the expected result") { (fields: List[(String, Json)]) =>
    val result1 = JsonObject.fromIterable(fields)
    val result2 = JsonObject.fromFoldable(fields)
  ...
  test("nonEmpty should return the expected result") { (fields: List[(String, Json)]) =>
    val result1 = JsonObject.fromIterable(fields)
    val result2 = JsonObject.fromFoldable(fields)
  ...
  test("kleisli should find fields if they exist") { (fields: List[(String, Json)], key: String) =>
    val result1 = JsonObject.fromIterable(fields)
    val result2 = JsonObject.fromFoldable(fields)
    val expected = fields.find(_._1 == key).map(_._2)
  ...
  property("keys should return all keys") {
    Prop.forAll { (fields: List[(String, Json)]) =>
      val result1 = JsonObject.fromIterable(fields)
      val result2 = JsonObject.fromFoldable(fields)
  ...
  property("toVector should return all fields") {
    Prop.forAll { (values: Vector[Json]) =>
      val fields = values.zipWithIndex.map {
        case (value, i) => i.toString -> value
      }.reverse
  ...
  property("add should replace existing fields with the same key") {
    Prop.forAll { (head: Json, tail: List[Json], replacement: Json) =>
  ...
  property("add, +:, and remove should be applied correctly") {
    Prop.forAll { (original: JsonObject, operations: List[Either[String, (String, Json, Boolean)]]) =>
  ...
  property("mapValues should transform the JSON object appropriately") {
    Prop.forAll { (values: List[Json], replacement: Json) =>
      val fields = values.zipWithIndex.map {
        case (value, i) => i.toString -> value
      }
  ...
  property("Eq[JsonObject] should be consistent with comparing fields") {
    Prop.forAll { (fields1: List[(String, Json)], fields2: List[(String, Json)]) =>
  ...
  property("deepMerge should merge correctly") {
    Prop.forAll { (left: JsonObject, right: JsonObject) =>
      ...
      merged.toList.foreach {
        case (key, value) =>
          (left(key), right(key)) match {
            ...
            case _                               => fail("Impossible state reached in deepMerge test")
          }
      }
    }
  }
