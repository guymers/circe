...
class EncoderSuite extends CirceSuite {
  ...
  property("mapJson should transform encoded output") {
    Prop.forAll { (m: Map[String, Int], k: String, v: Int) =>
  ...
  property("encodeMap should preserve insertion order") {
    Prop.forAll { (m: SortedMap[MyString, String]) =>
