...
class JsonNumberSuite extends CirceSuite {
  property("fromString should parse valid JSON numbers") {
    Prop.forAll { (jns: JsonNumberString) =>
  ...
  property("toBigDecimal should produce correct results") {
    Prop.forAll { (n: JsonNumber) =>
      (n != Json.fromDouble(-0.0).flatMap(_.asNumber).get) ==> {
  ...
  property("JsonFloat.toBigInt should return None if it loses precision") {
    Prop.forAll { (f: Float) =>
      val j = JsonFloat(f)
